        ...
        function render(req, res) {
            ...
            try {
                ...
                if( filteredQueries.length !== 1 ){
                    ...
                    res.status(500)
                    res.render( "error", {message : "Number of queries in database with title "+req.params.title+" was not exactly 1", redirect: '/' } );
                    return;
                ...
                if( query.executions.length < 1 ){
                    ...
                    //no content provided, so we return with 204 but redirect to an error page.
                    errorMessage = 'no executions for: ' + req.params.title + ' on EP: ' + req.params.endpoint;
                    logger.error('visualization/:title/:endpoint/:provider:' + errorMessage);
                    res.status(204)
                    res.render( "error", { message : "There were not executions found for query: "+req.params.title , redirect : '/'} );
                    return;
                ...
                barVisQueries        = ['PDC-053', 'PDC-054','PDC-056', 'PDC-057', 'PDC-058', 'PDC-1178'],
                ...
                if(req.visualization.script === null || req.visualization.script === undefined ){
                    ...
                    res.status(500); 
                    res.render('error', { message : "Could not find a visualization for query: "+ req.params.title, redirect : '/' } );
                    return;
                ...
                fs.readFile('./visualizations/' + req.visualization.script + '.html', 
                    function callback(err, script) {
                        ...
                        if (err) {
                            ...
                            res.status(500);
                            res.render( 'error', { message : 'Could not load visualization for query : '+req.params.title, redirect : '/'} ); 
                            return; 
                ...
                res.status(500);
                res.render('error', { message : "An exception occurred while trying load query: "+ req.params.title+".", redirect : '/' } );
                return; 
          ...
          data.middleware.hubapiGet(
            ...
            function complete(error, response){
              ...
              switch(error){
                ...
                case 400:
                  res.render("error", {message : "routes: Failed to fetch report!", redirect : '/'})
                  break; 
                case 401: 
                  res.render("error", {message : "routes: Unable to verify identity, either the session expired or invalid there are credentials.", redirect : '/auth'})
                  break;
                case 500: 
                  res.render("error", {message : "routes: Failure", redirect : '/'})
                  break; 
                default: 
                  res.render("error", {message : "routes: Failure", redirect : '/'})
                  break; 
              ...
              if( !response || !response.body ){
                ...
                res.render("error", {message : "Failure", redirect : '/'})
    ...
    router.route('/auth')
        ...
        .get(function (req, res) {
          ...
          try{
            ...
            res.status(500);
            res.render('error', {message : 'Internal server error: '+util.inspect(e, false, null), redirect:'/auth'});

    ...
    router.get('/visualization/:title/:endpoint/:provider',
        ...
        function render(req, res) {
          ...
          else
          {
            ...
            var sortedExecutions = query.executions.sort(
              function(a,b)
              {
            ...
            console.log('sortedExecutions: ' + util.inspect(sortedExecutions));

            var execution;

            sortedExecutions.every(
              function(x)
              {
                if(x.aggregate_result !== null && x.aggregate_result !== undefined)
                {
                  execution = x;
                  return false;
                }
            ...
            {
              logger.error('no execution with aggregate results');
              return;
            }

            console.log('query.executions: ' + util.inspect(query.executions, false, null));
            console.log('query.executions.length-1: ' + (query.executions.length-1));
            console.log('query.executions[query.executions.length-1]: ' + util.inspect(query.executions[query.executions.length-1], false, null));
            console.log('query.executions[query.executions.length-1].aggregate_result: ' + util.inspect(query.executions[query.executions.length-1].aggregate_result, false, null));

            req.visualization.script = 'BarVis';
          }

          console.log('vis data:');
          console.log(util.inspect(req.visualization, false, null));

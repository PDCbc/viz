          ...
          data.middleware.hubapiGet(
            ...
            function complete(error, response){
              ...
              switch(error){
                ...
                case 400:
                  res.render("error", {message : "Failed to fetch report!"})
                  break; 
                case 401: 
                  res.render("error", {message : "Unable to verify identity, either the session expired or invalid there are credentials."})
                  break;
                case 500: 
                  res.render("error", {message : "Failure"})
                  break; 
                default: 
                  res.render("error", {message : "Failure"})
                  break; 

...
function environment(next) {
    ...
    if (!process.env.PORT)
    {
      ...
      logger.warn('No $PORT present. Choosing a sane default, 9081.');
    ...
    if (!process.env.HUBAPI_URL) {
      ...
      logger.warn('No HUBAPI_URL present. Defaulting to `https://hubapi:9080`.');
      process.env.HUBAPI_URL = 'https://hubapi:9080';
    }
    else
    {

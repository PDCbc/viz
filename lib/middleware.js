...
function middleware(next, data) {
  var default_months = 36;
    ...
    function hubapiGet(req, path, callback) {
      ...
      logger.log('hubapiGet: ' + process.env.HUBAPI_URL + path);
    ...
    function populateVisualization(req, res, callback) {
        ...
        hubapiGet(req, '/api/processed_result/' + req.params.title + '/54d147f6f2ba326114000002/45567/', function validation(error, request) {
...
//27542
    ...
    */
    function populateIntegrityVisualization(req, res, callback) {
        if (!req.params.title) { return res.redirect('/'); }
        // TODO: Don't use a static.
        hubapiGet(req, '/integrity/' + req.params.title + '/54d147f6f2ba326114000002/45567/' + default_months, function validation(error, request) {
          if (error) { return next(error); }
            req.visualization = request.body;
            callback();
        });

          ...
          function (err, body){
            ...
            if( err ){
              ...
              if(!(body && body.federation && body.jurisdiction && body.username)){
                ...
                logger.warn("User does not exist in auth."); 
                next(401, null);
                return;

              }

              logger.log("Making request to hubapi: "+path);
              ...
              if (error) {  //an error code 400, 401, 500...
                ...
                switch(error){
                  ...
                  case 401:
                    ...
                    res.render('error', { message : "middleware.populateVisualizationList failed due to failure to authenticate."}); 
                    break;

                  case 500:

                    res.render('error', {message : 'middleware.populateVisualizationList failed due to a server failure.'}); 
                    break;

                  default:

                    res.render('error', { message : "middleware.populateVisualizationList failed" }); 
                    break; 
                ...
                if ( err ) {
                  ...
                  switch( err ){
                    ...
                    case 401: 
                      ...
                      res.statusCode = 401; 
                      res.render('auth', {message : 'Invalid username and password combination, please try again. '}); 
                    ...
                    if( !authResult || !authResult.cookie ){
                      ...
                      res.statusCode = 500; 
                      res.render('error', {message : "Could not obtain a session from auth."}); 
                      return;
                      ...
                      auth.verifyAuth(authResult.cookie, function (err, user) {
                          ...
                          if ( err ){
                            ...
                            delete req.session.bakedCookie;
                            res.statusCode = 500; 
                            res.render('error', {message : "Could not verify session with auth."}); 

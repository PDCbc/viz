...
function middleware(next, data) {
  var default_months = 36;
    ...
    function populateVisualization(req, res, callback) {
        ...
        hubapiGet(req, '/api/processed_result/' + req.params.title + '/54bea239ff60a6dd0c000002/45567/', function validation(error, request) {
...
//27542
    ...
    */
    function populateIntegrityVisualization(req, res, callback) {
        if (!req.params.title) { return res.redirect('/'); }
        // TODO: Don't use a static.
        hubapiGet(req, '/api/integrity/' + req.params.title + '/54bea239ff60a6dd0c000002/45567/' + default_months, function validation(error, request) {
          if (error) { return next(error); }
            req.visualization = request.body;
            callback();
        });

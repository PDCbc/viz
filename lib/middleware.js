...
function middleware(next, data) {
  var default_months = 36;
    ...
    function hubapiGet(req, path, callback) {
      ...
      console.log('cookie for ajax request is: ' + req.session.bakedCookie);
            ...
            require('request').get({ url: process.env.HUBAPI_URL + path + '?cookie=' + req.session.bakedCookie, json: true },
              function (error, response, body) {
                ...
                if(error || response.statusCode !== 200)
                {
                  ...
                  logger.error('HUBAPI_GET: ' + errorResponse);
                  callback(errorResponse, null);
                  return;

                }

                logger.success('got back from hubapi call without an error: body - ' + util.inspect(body, false, null));
    ...
    function populateVisualizationList(req, res, callback) {
        ...
        logger.log('popvislist: visualization - req.visualization - ' + util.inspect(req.visualization, false, null));
    ...
    function populateVisualization(req, res, callback) {
        ...
        hubapiGet(req, prefix  + req.session.user.clinic + '/' + req.session.user.clinician + '/', function validation(error, request) {
            ...
            logger.warn('pop vis -  callback for hubapi');
    ...
    */
            ...
            auth.getBakedCookie(req.body.juri, req.body.user, req.body.password, function (err, bakedCookie) {
                ...
                if (err) {
                    ...
                    auth.verifyAuth(bakedCookie, function (err, user) {
                        ...
                        console.log('AUTHENTICATE_USER: req.session.user: ' + util.inspect(req.session.user, false, null));
                        callback();
                    });
                }
            });
    ...
    function checkAuthentication(req, res, callback) {
      logger.warn('You are definitely in checkAuthentication');
